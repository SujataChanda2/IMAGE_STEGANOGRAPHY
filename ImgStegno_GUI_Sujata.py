...
class IMG_Stegno:
    ...
    def main(self, root):
        root.title('ImageSteganography by Sujata')
        ...
        title = Label(frame,text='Image Steganography by Sujata')
        title.config(font=('Times new roman',25, 'bold'))
        title.grid(pady=0)
        title.config(fg='#FF0000',bg = '#Ffffff')
        title.grid(row=1)

        encode = Button(frame,text="Encode",command= lambda :self.encode_frame1(frame), padx=14,bg = '#000000' )
        encode.config(font=('Helvetica',14), bg='#000000',fg='#FF0000')
        encode.grid(row=2)
        decode = Button(frame, text="Decode",command=lambda :self.decode_frame1(frame), padx=14,bg = '#000000')
        decode.config(font=('Helvetica',14), bg='#000000',fg='#FF0000')
        decode.grid(pady = 12)
        decode.grid(row=3)
    ...
    def encode_frame1(self,F):
        ...
        button_bws.config(font=('Helvetica',18), bg='#FF0000')
        button_bws.grid()
        button_back = Button(F2,  text='Cancel', command=lambda : IMG_Stegno.back(self,F2))
        button_back.config(font=('Helvetica',18),bg='#FF0000')
    ...
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            text_a.grid()
            encode_button = Button(e_pg, bg= 'FF0000', text='Cancel', command=lambda : IMG_Stegno.back(self,e_pg))
            encode_button.config(font=('Helvetica',14), bg='#FF0000')
            data = text_a.get("1.0", "end-1c")
            button_back = Button(e_pg,bg= 'FF0000',  text='Encode', command=lambda : [self.enc_fun(text_a,my_img),IMG_Stegno.back(self,e_pg)])
            button_back.config(font=('Helvetica',14), bg='#FF0000')
            button_back.grid(pady=15)
            encode_button.grid()
            e_pg.grid(row=1)
            e_F2.destroy()
